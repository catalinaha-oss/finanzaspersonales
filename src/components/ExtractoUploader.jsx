...
const EDGE_URL = 'https://puvqxgsodwyhfdfnatpv.supabase.co/functions/v1/analizar-extracto'
      ...
      <div className="modal" style={{ maxWidth:520 }}>
        ...
        {(estado === 'analizando' || estado === 'guardando') && (
          <div style={{ textAlign:'center', padding:'2rem 0' }}>
            <div style={{ width:36, height:36, border:'3px solid var(--bg4)', borderTopColor: estado==='guardando'?'var(--green)':'var(--accent)', borderRadius:'50%', animation:'spin 0.8s linear infinite', margin:'0 auto 16px' }} />
            <p style={{ color:'var(--text2)', fontSize:'0.85rem' }}>
              {estado === 'analizando' ? 'Analizando extracto con IA...' : 'Guardando en Supabase...'}
            </p>
            {estado === 'analizando' && <p style={{ color:'var(--text3)', fontSize:'0.75rem', marginTop:4 }}>Esto puede tardar unos segundos</p>}
        ...
        {estado === 'listo' && resultado && (
          <div style={{ textAlign:'center', padding:'1.5rem 0' }}>
            ...
            <p style={{ color:'var(--text2)', fontSize:'0.85rem', marginBottom:4 }}>{resultado.creadas} deuda{resultado.creadas!==1?'s':''} nueva{resultado.creadas!==1?'s':''}</p>
            <p style={{ color:'var(--text2)', fontSize:'0.85rem', marginBottom:'1.5rem' }}>{resultado.actualizadas} deuda{resultado.actualizadas!==1?'s':''} actualizada{resultado.actualizadas!==1?'s':''}</p>
